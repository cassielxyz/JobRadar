...
def copilot_review(job: Job, category: Category) -> AIResult:
    ...
    try:
        proc = subprocess.run(
            ["copilot", "-p", _prompt(job, category), "-s", "--no-ask-user", "--no-custom-instructions"],
...
def merge_ai_score(base_score: int, reasons: list[str], eligible: bool, review: dict[str, Any], category: Category | None = None):
    ...
    if fresher is False and agreement and confidence >= 80:
        score -= 20
        if category is not None and getattr(category, 'fresher_only', False):
            eligible = False
            reasons.append("fresher-only category: AI detected experience requirement above target")
        else:
            reasons.append("AI detected experience above preferred target")
