...
export async function GET(){const a=await auth();if(!a)return NextResponse.json({error:'Unauthorized'},{status:401});const {data,error}=await admin().from('generated_resumes').select('*').eq('user_id',a.user!.id).order('updated_at',{ascending:false});if(error)return NextResponse.json({error:error.message},{status:500});return NextResponse.json(await Promise.all((data||[]).map(enrichRow)))}
...
export async function POST(req:Request){
  ...
  const {data,error}=await admin().from('generated_resumes').upsert(payload,{onConflict:'user_id,job_id,base_resume_id'}).select().single();return NextResponse.json(error?{error:error.message}:await enrichRow(data),{status:error?400:201});
}
...
export async function PATCH(req:Request){
  ...
  const {data:existing}=await admin().from('generated_resumes').select('*').eq('id',id).eq('user_id',a.user!.id).maybeSingle();if(!existing)return NextResponse.json({error:'Generated resume not found'},{status:404});
  ...
  const incoming={...(body.content_json||{})};if('skills'in incoming)incoming.skills=safeArr(incoming.skills).filter((x:string)=>allowedSkill(x,verified));
...
export async function DELETE(req:Request){const a=await auth();if(!a)return NextResponse.json({error:'Unauthorized'},{status:401});const id=new URL(req.url).searchParams.get('id');if(!id)return NextResponse.json({error:'id required'},{status:400});const {error}=await admin().from('generated_resumes').delete().eq('id',id).eq('user_id',a.user!.id);return NextResponse.json(error?{error:error.message}:{ok:true},{status:error?500:200})}
